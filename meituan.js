...
class Task {
    ...
    async DoTask(body) {
        try {
            ...
            if (body.taskType == 1) {
                if ("classify" in body) {
                    if (body.classify == 2) {
                        taskStatusResult = await this.taskRequest_task("post", `https://app.dewu.com/hacking-task/v1/task/pre_commit?sign=b7382f4d908e04356f9646688afe096c`, {
                            taskId: body.taskId,
    ...
    async TaskReceiveFish(body) {
        try {
            let result = await this.taskRequest("post", `https://app.dewu.com/hacking-fish/v1/task/receive?sign=ee632e4b8e24d2526737bca0b7c0c678`, {
                taskId: body.taskId,
    ...
    async TaskReceiveTree(body) {
        try {
            let result = await this.taskRequest("post", `https://app.dewu.com/hacking-tree/v1/task/receive?sign=15c051cc7af50c30318c05b539e434e7`, {
                taskId: body.taskId,
                classify: body.classify
            })
            //console.log(JSON.stringify(result));
        ...
        try {
            ...
            if (this.index == 1) {
                for (let j of authShareCodeList) {
                    authHelpCode = j
                    result = await this.taskRequest("post", `https://app.dewu.com/hacking-tree/v1/user/init?sign=d25c30ebdf1adeb29ca10ccb825bbf66`, {
                        "keyword": "",
                        "source": "wotab04",
                        "koc": 0,
                        "ffOfflineFlag": "",
    ...
    randomStr(length, charset) {
        var tmp1, tmp2, data = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".split(""), result = [];
        if (((charset = charset || data["length"]), length)) for (tmp1 = 0; tmp1 < length; tmp1++) result[tmp1] = data[0 | (Math.random() * charset)]; else for (result[8] = result[13] = result[18] = result[23] = "-", result[14] = "4", tmp1 = 0; tmp1 < 36; tmp1++) result[tmp1] || ((tmp2 = 0 | (16 * Math["random"]())), (result[tmp1] = data[19 === tmp1 ? (3 & tmp2) | 8 : tmp2]));
        return result["join"]("")
    }
                ...
                if (value.length > 0) {
                    ...
                    if (typeOfFirstItem === 'object') {
                        ...
                        value.forEach((item, index) => {
                            arrayStr += JSON.stringify(item) + (index !== value.length - 1 ? ',' : '')
                        });
